function [zt,TT,z_samps,Ts, mean_u,cov_u, samp_U,samp_R,mean_samps,cov_samps, mom_u] = triad_stocstat_samps(params, m0, var0)
...
mean_u(:,ts)=Ut;
cov_u(:,:,ts)=Rt;
% samples with small ensemble
Ut_samp=repmat(Ut,[1 nsamp]);
Rt_samp=repmat(Rt,[1 1 nsamp]);
mean_samps(:,:,ts)=Ut_samp;
cov_samps(:,:,:,ts)=Rt_samp;
% save samples
ic=1; ns=floor(.5/Dt);
for ii=1:Nt
    ...
    if ~mod(ii,tstep)
        ...
        % small samps updates
        for ss=1:nsamp
            zsamp1(:,:)=zs_pre(1+(ss-1)*MC1:ss*MC1,:);
            fz_samp = Buu(B,zsamp1,zsamp1);
            Rt_samp1=Rt_samp(:,:,ss);
            for i=1:3
                for j=1:3
                    Lvs(i,j)=(Buu2(B,Ut_samp(:,ss),Bsis(:,j))+Buu2(B,Bsis(:,j),Ut_samp(:,ss))+(L-G)*Bsis(:,j))'*Bsis(:,i);
                    Q_samp(i,j)=mean(fz_samp(:,i).*zsamp1(:,j));
                end
            end
            dU_samp=Buu2(B,Ut_samp(:,ss),Ut_samp(:,ss))-G*Ut_samp(:,ss)+L*Ut_samp(:,ss) + mean(fz_samp)'+params.tmM(ii,:)';
            dR_samp=Lvs*Rt_samp1+Rt_samp1'*Lvs'+SS^2+Q_samp+Q_samp' +params.eps*(cov(zsamp1)-Rt_samp1);

            samp_U(:,  ss,ts-1)=dU_samp-dUt;
            samp_R(:,:,ss,ts-1)=dR_samp-dRt;
    
            Ut_samp(:,  ss)=Ut_samp(:,  ss)+tstep*Dt*dU_samp;
            Rt_samp(:,:,ss)=Rt_samp(:,:,ss)+tstep*Dt*dR_samp;
        end
        mean_samps(:,:,ts)=Ut_samp;
        cov_samps(:,:,:,ts)=Rt_samp;
    end
    if mod(ii,ns)==0
        ...
        display(['Samps iter = ',num2str(ii),': mean = ',num2str(mean_u(:,ts)')]);
    end
end